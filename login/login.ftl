...
<@layout.registrationLayout displayInfo=social.displayInfo; section>
    ...
    <#elseif section = "form">
        <#if realm.password>
            <form id="kc-form-login" class="${properties.kcFormClass!''}" action="${url.loginAction}" method="post" class="form-signin">
                <div class="${properties.kcFormGroupClass!}">
                    ...
                    <div class="${properties.kcInputWrapperClass!}">
                        <#if usernameEditDisabled??>
                            <input id="username" class="${properties.kcInputClass!} ${messagesPerField.printIfExists('username',properties.kcFormGroupErrorClass!)}" name="username" value="${(login.username!'')}" type="text" disabled placeholder="${username_label}"/>
                        <#else>
                            <input id="username" class="${properties.kcInputClass!} ${messagesPerField.printIfExists('username',properties.kcFormGroupErrorClass!)}" name="username" value="${(login.username!'')}" type="text" autofocus autocomplete="off" placeholder="${username_label}" />
                        </#if>
                    </div>
                </div>
                ...
                <div class="${properties.kcFormGroupClass!}">
                    <div class="${properties.kcLabelWrapperClass!}">
                        <label for="password" class="${properties.kcLabelClass!} ${messagesPerField.printIfExists('password',properties.kcFormGroupErrorClass!)}">${msg("password")}</label>
                    </div>

                    <div class="${properties.kcInputWrapperClass!}">
                        <input id="password" class="${properties.kcInputClass!} ${messagesPerField.printIfExists('password',properties.kcFormGroupErrorClass!)}" name="password" type="password" autocomplete="off" placeholder="${msg('password')}" />
                    </div>
                </div>
    ...
    <#elseif section = "info" >
        <#if realm.password && social.providers??>
            ...
            <div id="kc-social-providers">
                <h2 class="h5 mb-3 font-weight-normal">${msg("socialProviders")}</h2>
                <ul class="list">
